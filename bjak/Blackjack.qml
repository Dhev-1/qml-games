//  Blackjack.qml — six-deck blackjack, the whole widget in one component.
//
//  The game itself. blackjack.qml wraps this in a ShellRoot to run it on its
...
Scope {
    ...
    // True only under blackjack.qml, where this is the whole process: there,
    // closing is quitting, as it always was. Inside edge the shell outlives the
    // game, so closing only hides it — and the window is destroyed either way,
    // so the shoe and the felt stop costing anything the moment it goes.
    ...
    property QtObject theme: QtObject {
        ...
        //  Where it sits, and how long it takes to get there — the same numbers
        //  poker rests on, so the two widgets land in the same corner. 44 is
        //  edge's bar down the right, 10 its border along the bottom, and the
        //  remainder is breathing room. Zero them on a bare compositor.
    ...
    //  IPC
    //
    //  Standalone, closing still quits: there is no state worth keeping a whole
    //  QML engine resident for — the bank is on disk, and the shoe is 312
    //  shuffled cards that mean nothing between rounds. Inside edge the process
    //  is the shell's and outlives any one game, so closing lowers `open` and
    //  the LazyLoader drops the window — which is where the felt, the shoe's
    //  painted cards and the rack actually live.
    // ═══════════════════════════════════════════════════════════
    function quit(): void {
        if (root.standalone)
            Qt.quit();
    ...
    //  WINDOW
    //
    //  A layer surface in the bottom-right corner, slid up into view — poker's
    //  block, because two widgets that share a corner should arrive the same
    //  way. Everything below `frame` is untouched by it.
    // ═══════════════════════════════════════════════════════════
    ...
    LazyLoader {
        ...
        PanelWindow {
            ...
            WlrLayershell.namespace: "blackjack"
            ...
            //  than the window being moved, so the layer surface is laid out
            //  once and the compositor isn't resizing it sixty times a second.
