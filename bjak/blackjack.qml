...
ShellRoot {
    ...
    //  Returns whether the chip went down, which is what stops a held button
    //  from spinning on an empty bank.
    function placeBet(): bool {
        if (!root.wager(root.chips[root.chipIndex])) return false;
        root.message = "PLACE YOUR BET";
        return true;
    }

    //  Returns whether a chip actually came back off, so a held button knows
    //  when the circle is empty — the mirror of placeBet.
    function undo(): bool {
        if (root.phase !== "betting" || root.betOrder.length === 0) return false;
                    ...
                    Item {
                        ...
                        Item {
                            ...
                            MouseArea {
                                ...
                                //  On press rather than on click, so holding
                                //  carries on from the same event that laid
                                //  the first chip down instead of waiting for
                                //  a release that is never coming. Both
                                //  buttons repeat: left stacks chips up, right
                                //  takes them back off.
                                onPressed: (m) => {
                                    repeatBet.removing = m.button === Qt.RightButton;
                                    if (repeatBet.step()) {
                                        repeatBet.interval = 340;
                                        repeatBet.restart();
                                    }
                                }
                                onReleased: repeatBet.stop()
                                onCanceled: repeatBet.stop()
                            }

                            //  Hold to keep going, in whichever direction the
                            //  button that started it means. The first gap is
                            //  the pause that tells a click from a hold; after
                            //  it the chips move at speed. It stops itself the
                            //  moment one will not move — the bank being short,
                            //  the circle being empty, or the round having moved
                            //  on — so a held button cannot sit there failing in
                            //  a loop.
                            Timer {
                                id: repeatBet
                                ...
                                function step(): bool {
                                    return repeatBet.removing ? root.undo()
                                                              : root.placeBet();
                                }
                                ...
                                interval: 340
                                onTriggered: {
                                    repeatBet.interval = 70;
                                    if (!repeatBet.step()) repeatBet.stop();
                                }
                            }
                        }
