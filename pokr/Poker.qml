...
Scope {
    ...
    //  Whether closing quits or only hides, and the screen to open on. Both as
    //  in Blackjack.qml.
    ...
    //  The rack: the base four always out, higher denominations as the bankroll
    //  reaches them. See Blackjack.qml.
    ...
    //  wears the hundred's. See Blackjack.qml.
    ...
    // ═══════════════════════════════════════════════════════════
    //  THE BANK ON DISK — see Blackjack.qml for the full note. `statePath`
    //  hashes the canonical config path, so each game lands on its own file and
    //  the banks stay separate without arranging it.
    // ═══════════════════════════════════════════════════════════
    FileView {
        id: bankFile
        ...
        //  Written synchronously, so the last write lands before `Qt.quit()`
        //  returns. See Blackjack.qml.
        blockWrites: true
        atomicWrites: true
    ...
    component Chip: Item {
        ...
        Canvas {
            ...
            onPaint: {
                ...
                //  outside it, so a picked chip takes up no more room. See
                //  Blackjack.qml.
                ctx.beginPath();
                ctx.arc(cx, cy, rim, 0, 2 * Math.PI);
        ...
        Text {
            ...
            //  Sized to the face inside the ring, not the whole chip, so the
            //  number never runs under the spots. See Blackjack.qml for where
            //  0.66 / 0.6n comes from.
